...
export default function CalendarScreen() {
  ...
  const renderCalendar = () => {
    ...
    const totalSlots = days + firstDay;
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        if (day > 0 && day <= days) {
          ...
          week.push(<View key={`empty-${index}`} style={styles.calendarDay} />);
        }
      }
      calendar.push(
  ...
  const renderMedicationsForDate = () => {
    ...
    const isFuture = new Date(new Date().setHours(0,0,0,0)) < new Date(new Date(selectedDate).setHours(0,0,0,0));

    // Filtrar medicamentos activos para la fecha seleccionada
        ...
        <View key={medication.id} style={styles.medicationCard}>
          ...
          {taken ? (
            ...
            <TouchableOpacity
              ...
              disabled={isFuture}
              onPress={async () => {
                // Si es hoy, usar hora actual. Si es pasado, usar mediodía de ese día.
                const timestamp = new Date().toDateString() === selectedDate.toDateString() 
                  ? new Date().toISOString() 
                  : new Date(new Date(selectedDate).setHours(12,0,0,0)).toISOString();
                
                await recordDose(medication.id, true, timestamp);
                loadData();
              }}
            >
